...
## Usage
Use this link [https://birkeeper.github.io/Choir-practice-midi-player/midi_player.html](https://birkeeper.github.io/Choir-practice-midi-player/midi_player.html) to start the midi player. It should work in many different browsers on many operating systems, including Android, iOS, Windows and MacOS. It can play a song in the background, so a digital music sheet can be consulted simultaneously in a different app. You can also control the midi player from the media session in your operating system's notification or home screen. In addition, the player can be controlled from the controls on your earbuds or headphones.

The first time you run it, it takes some time to download the soundfont before you can interact with the application. A Wi-Fi connection is recommended to shorten the waiting time. After that the application will respond quicker, because the soundfont is stored in cache. After the installation, the application can also run offline.
...
<p align='center'>
    <img src="./icons/Screenshot_Android.png" style="display: block; margin: auto; width: 50%;" />
</p>
